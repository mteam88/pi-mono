...
Legacy entry created by older pi versions when switching branches via `/tree` with an LLM generated summary of the left branch up to the common ancestor. New summaries are stored as `ContextRewriteEntry` insertions, but existing `branch_summary` entries still project into context.
...
Append-only branch-local rewrite of the projected context view. It does not mutate previous session entries. `buildSessionContext()` applies active rewrites on the current branch after base projection and before returning messages for the LLM.

Targets:
- `range`: replace projected entries from `fromEntryId` through `toEntryId` with `after`
- `surface`: replace a text surface on one entry (`text`, `output`, `summary`, or whole `rendered` entry)
- `insert`: insert `after` after an entry, or at the start when `afterEntryId` is `null`

```json
{"type":"context_rewrite","id":"r1s2t3u4","parentId":"a1b2c3d4","timestamp":"2024-12-03T14:16:00.000Z","rewriteId":"omit-large-output","target":{"kind":"surface","entryId":"c3d4e5f6","surface":"output"},"beforeHash":"sha256:...","after":"[large output omitted]","reason":"not needed in future context"}
```

Undo is append-only too:

```json
{"type":"context_rewrite_undo","id":"u1v2w3x4","parentId":"r1s2t3u4","timestamp":"2024-12-03T14:17:00.000Z","rewriteId":"omit-large-output"}
```
...
4. Converts legacy `BranchSummaryEntry` and `CustomMessageEntry` to appropriate message formats
5. Applies active `ContextRewriteEntry` entries on the current branch, excluding rewrites later undone by `context_rewrite_undo`
...
- `appendContextRewrite(rewrite)` - Add a branch-local context rewrite
- `appendContextRewriteUndo(rewriteId)` - Undo a context rewrite on the current branch
...
- `branchWithSummary(entryId, summary, details?, fromHook?)` - Branch with context summary stored as a context rewrite insertion

### Instance Methods - Context & Info
- `buildSessionContext()` - Get messages, thinkingLevel, and model for LLM
- `buildSessionProjection()` - Get context messages with source entry IDs and active context rewrites
